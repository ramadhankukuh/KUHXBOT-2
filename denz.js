...
try {
		...
		const ownerNumberr = ["12816245470@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
        ...
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fH', "groupName": `${NamaBot}`, "footerText": "*_© KUHXBOT_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
denz.updatePresence(from, Presence.recording)
   ...
   const sotoy = [
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
       ...
       menu = `${uwu}
╭─❒ 「 COMMAND 」 ❒
│ ⬡ Untuk Memilih Menu Bisa Klik Button *LIST MENU*
└❏
...
└❏
${uwu}`
sendButLocation(from, `${menu}`, "*_© KUHXBOT_*", {jpegThumbnail:ofrply}, [{buttonId:`${prefix}command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`${prefix}topup`,buttonText:{displayText:'TOPUP GAME'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
...
case 'pulsa':
 ...
 footerText: '*_© KUHXBOT_*',
 description: `Hai kak @${stod.split('@')[0]}, Silahkan pilih provider disini`,
 sections: [
                     {
                      "title": `${jmn} - ${week} ${weton} - ${calender}`,
 rows: [
...
case 'pulsaxl':
 ...
 rows: [
                          {
                              "title": "100.000 = 98.000",
                              "rowId": ""
                           },
                           {
                              "title": "200.000 = 195.000",
                              "rowId": ""
                           },
                           {
                              "title": "500.000 = 480.000",
                              "rowId": ""
							}
                        ]
                     }],
 ...
 rows: [
                          {
                           ...
                           {
                              "title": "100.000 = 95.000",
                              "rowId": ""
                           },
                           {
                              "title": "200.000 = 190.000",
                              "rowId": ""
							}
                        ]
                     }],
...
client.browserDescription = ['Kuh','Desktop','3.0']
...
break
		...
		case 'sc':
		case 'sourcecode':
		denz.sendMessage(from, { text: "https://github.com/ramadhankukuh", matchedText: 'https://github.com/ramadhankukuh', description: "", title: "don't click here !!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
	...
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  ...
	  denz.sendMessage(from, delb, MessageType.image, {caption: oi1})
                ...
                */
					...
					case 'tahta':  
					if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} kukuh*`)
					reply(mess.wait)
					anu = await getBuffer(`${ApiZeks}/api/hartatahta?text=${F}&apikey=${zeksApikey}`)
					denz.sendMessage(from, anu, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done`, quoted: mek})
					break
		case 'neon2':  
					if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} kukuh*`)
					reply(mess.wait)
		...
		case 'wolf':  
					if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} kukuh*`)
					reply(mess.wait)
					anu = await getBuffer(`${ApiZeks}/api/wolflogo?apikey=${zeksApikey}&text1=kukuhofc&text2=${F}`)
					denz.sendMessage(from, anu, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done`, quoted: mek})
					break 
		case 'tfire':  
					if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} kukuh*`)
		...
		case 'nulis':  
		 ...
		 denz.sendMessage(from, anu1, image, {quoted: ftroli, caption : '.sticker'})
		break
	case 'halloween':
...
.on('end', function () {
console.log('DONE')
denz.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek1})
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
break
			...
			case 'ping':
				...
				case 'nulis':
					...
					reply('wait a minute...')
									 ...
									 denz.sendMessage(from, rakz, image, { quoted: mek ,thumbnail: Buffer.alloc(0) });
			...
			case 'add':
				...
				if (args[0].startsWith('08')) return reply('Gunakan kode negara mas/mba')
				orang = args[0] + '@s.whatsapp.net'
response = await denz.groupAdd(from, [orang])
...
denz.sendMessage(from, orang, inv[0].invite_code, inv[0].invite_code_exp, groupMetadata.subject , `Salah Satu Admin Mengundang Anda Masuk Ke Sini Silahkan Klik Bergabung Untuk Masuk`)
				...
				break 
				case 'kick':
					if (!isGroup) return denz.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
					if (!isGroupAdmins) return denz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
					if (!isBotGroupAdmins) return denz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
					if (mentionedJidList.length === 0) return denz.reply(from, 'Untuk menggunakan Perintah ini, kirim perintah *#kick* @tagmember', id)
					await denz.sendText(from, `Perintah diterima, mengeluarkan:\n${mentionedJidList.join('\n')}`)
					for (let i = 0; i < mentionedJidList.length; i++) {
						if ((adminNumber, groupAdmins).includes(mentionedJidList[i])) return denz.reply(from, mess.error.Sp, id)
						await denz.removeParticipant(groupId, mentionedJidList[i])
					}
					break
                    case 'creategroup':
			case 'creategrup':
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer KUHXBOT",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/12816245470?text=Assalamualaikum"}}})
	}
    }
    }
    }
